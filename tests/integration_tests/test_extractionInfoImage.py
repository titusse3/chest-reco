...
def test_extraction_info_image_work(template, number):
  ...
  result = number_extractor(extraction, X_IMG)
  assert result is not None, f"Le nombre n'a pas pu être extrait."

  n = number_ocr(result)
  assert n is not None, f"Le nombre extrait est incorrect"
